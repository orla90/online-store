...
const valueObjForLocalStorage = (JSON.parse(JSON.stringify(valueObjDefault)));
const valueObjFromLocalStorage = JSON.parse(JSON.stringify(valueObjDefault));
...
const selectClickedManufacturerFilter = (
  ...
  const data = (clickedManufacturerFilter as HTMLDivElement).getAttribute(
    'data-manufacturer',
  );
  if (
...
const selectClickedColorFilter = (clickedColorFilter: EventTarget | null) => {
  const data = (clickedColorFilter as HTMLDivElement).getAttribute('data-color');
  ...
  rangeInput.forEach((input) => {
    input.addEventListener('input', (e: Event) => {
      ...
      if (maxValue - minValue < amountGap) {
        ...
        valueObjForLocalStorage.minAmountVal = minValue;
        valueObjForLocalStorage.maxAmountVal = maxValue;
  ...
  rangeInput.forEach((input) => {
    input.addEventListener('input', (e: Event) => {
      ...
      if (maxValue - minValue < amountGap) {
        ...
        valueObjForLocalStorage.minYearVal = minValue;
        valueObjForLocalStorage.maxYearVal = maxValue;
...
const addActiveClass = () => {
  ...
  const thumbs = document.querySelectorAll('input[type="range"]');
